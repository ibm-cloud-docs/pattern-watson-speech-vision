...
### High Availability for IBM Watsonx services and {{site.data.keyword.Bluemix_notm}} Software as a Service (SaaS) services
...
### Disaster Recovery for IBM Watsonx services and {{site.data.keyword.Bluemix_notm}} Software as a Service (SaaS) services

For the {{site.data.keyword.Bluemix_notm}} Watson and SaaS services used in this solution, the focus should be on preserving the data through an appropriate backup strategy.

In the unlikely event of an {{site.data.keyword.Bluemix_notm}}  multi-zone region failure, the services remain available in other multi-zone regions. Therefore the steps to recover from the failure would be to create new services instances in another multi-zone region and then to restore the data that was previously backed up to these new services instances.

#### Disaster Recovery for {{site.data.keyword.Bluemix_notm}} Speech to Text

Your disaster recovery plan includes knowing, preserving, and being prepared to restore all data that is maintained on {{site.data.keyword.Bluemix_notm}}. This includes data from custom language models, custom acoustic models, and asynchronous speech recognition requests.
...
#### Disaster Recovery for {{site.data.keyword.Bluemix_notm}} Text to Speech

For the Text to Speech service, only data for custom models, custom words, speaker models, and custom prompts is stored on {{site.data.keyword.Bluemix_notm}}. Your disaster recovery plan includes knowing, preserving, and being prepared to restore your customization information.
...
### Disaster Recovery for {{site.data.keyword.Bluemix_notm}} VPC Virtual Servers Instances

{{site.data.keyword.Bluemix_notm}} VPC virtual server instances and their storage are tied to the availability zone where they are deployed (where they are highly available).

To be able to restore {{site.data.keyword.Bluemix_notm}} VPC virtual server instances when a disaster affects an {{site.data.keyword.Bluemix_notm}} availability zone or a region, regular backups should be taken.

The native {{site.data.keyword.Bluemix_notm}} Backup for VPC service allows to automatically create backups and manually restore Block Storage for VPC volumes. It is based on the {{site.data.keyword.Bluemix_notm}} snapshots for VPC service.
...
For more information regarding {{site.data.keyword.Bluemix_notm}} backup for VPC, see [Backup for VPC] (https://cloud.ibm.com/docs/vpc?topic=vpc-backup-service-about&interface=ui).

### Disaster Recovery for ROKS

To protect Red Hat Openshift on {{site.data.keyword.Bluemix_notm}} workloads against an {{site.data.keyword.Bluemix_notm}} multi-zone region (MZR) wide failure, several clusters need to be deployed in different {{site.data.keyword.Bluemix_notm}} regions and be exposed via a global load balancer such as {{site.data.keyword.Bluemix_notm}} Internet Services (CIS).
...
### Disaster Recovery for {{site.data.keyword.Bluemix_notm}} Databases for PostgreSQL

As part of the service, a daily backup of the database is automatically taken. The backup is stored  in a cross-region Object Store instance. Additional manual backup can also be performed. In case of unavailability of the original {{site.data.keyword.Bluemix_notm}} region, the backup can be restored to a new instance of {{site.data.keyword.Bluemix_notm}} Databases for PostgreSQL.
...
In addition, provisioning Cloudant instances in different {{site.data.keyword.Bluemix_notm}} regions and configuring bidirectional replication between them allows to quickly recover to the latest state in case a disaster makes a region unavailable.

### Disaster Recovery for {{site.data.keyword.Bluemix_notm}} Code Engine

To protect Code Engine workloads against an {{site.data.keyword.Bluemix_notm}} multi-zone region (MZR) wide failure, the workloads should be deployed across multiple MZRs and implement an automatic failover mechanism by leveraging an Edge Proxy service such as {{site.data.keyword.Bluemix_notm}} Internet Services (CIS). For more information about deploying an application across multiple regions, see [Deploying code engine workloads across multiple regions](https://cloud.ibm.com/docs/codeengine?topic=codeengine-deploy-multiple-regions).
